...
): (toolName: string, toolInput: Record<string, unknown>) => Promise<{ behavior: 'allow' | 'deny'; message?: string }> {
  const deny = (msg: string) => Promise.resolve({ behavior: 'deny' as const, message: msg });
  const allow = () => Promise.resolve({ behavior: 'allow' as const });
