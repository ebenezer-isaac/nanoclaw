...
export interface SecurityPolicy {
  ...
  output: { strip_internal_tags: boolean; strip_markdown_images: boolean };
  killswitch: { file: string; enabled_value: string; message: string };
}
...
const DEFAULT_WRITE_BLOCKED: string[] = [
  '\\/skills\\/',
  'SOUL\\.md',
  'TOOLS\\.md',
  'IDENTITY\\.md',
...
const DEFAULT_POLICY: SecurityPolicy = {
  ...
  write: {
    ...
    trust_required_paths: [],
  },
  mounts: { readonly_overlays: DEFAULT_READONLY_OVERLAYS },
  output: { strip_internal_tags: true, strip_markdown_images: true },
  killswitch: {
    file: 'killswitch.txt',
...
export function loadSecurityPolicy(
  ...
  try {
    ...
    logger.warn(
      { err, path: filePath },
      'security-policy: cannot read config, using defaults',
    );
    return getDefaultPolicy();
  }
  ...
  if (output) {
    if (typeof output.strip_internal_tags === 'boolean') {
      policy.output.strip_internal_tags = output.strip_internal_tags;
    }
    if (typeof output.strip_markdown_images === 'boolean') {
      policy.output.strip_markdown_images = output.strip_markdown_images;
    }
  }
  ...
  if (killswitch) {
    ...
    if (typeof killswitch.enabled_value === 'string') {
      policy.killswitch.enabled_value = killswitch.enabled_value;
    }
...
export function readKillswitch(
  ...
  try {
    const content = fs.readFileSync(ksPath, 'utf-8').trim();
    if (content === policy.killswitch.enabled_value) {
      return { canRun: true, message: '' };
    }
    return { canRun: false, message: policy.killswitch.message };
  } catch {
    // File doesn't exist — fail-open (agent can run)
    return { canRun: true, message: '' };
  }
}
