...
describe('getDefaultPolicy', () => {
  ...
  it('write blocks CLAUDE.md and skills', () => {
...
describe('canUseTool (default rules)', () => {
  ...
  function testCanUseTool(
    ...
    const rules = { ...buildContainerSecurityRules(getDefaultPolicy()), ...extraRules };
    ...
    if (toolName === 'Bash') {
      ...
      if (rules.bash.blockedEnvVars.length > 0) {
        const envPattern = rules.bash.blockedEnvVars.map((v) => `\\$\\{?${v}\\}?`).join('|');
    ...
    if (toolName === 'WebFetch') {
      ...
      if (rules.webfetch.httpsOnly && url && !/^https:\/\//i.test(url)) return { allowed: false };
  ...
  it('blocks env piped', () => {
    expect(testCanUseTool('Bash', { command: 'env | cat' }).allowed).toBe(false);
  });

  it('blocks env in subshell', () => {
    expect(testCanUseTool('Bash', { command: 'echo $(env)' }).allowed).toBe(false);
  });
  ...
  it('blocks env with semicolon', () => {
    expect(testCanUseTool('Bash', { command: 'env;cat /etc/passwd' }).allowed).toBe(false);
  });
  ...
  it('blocks export -p', () => {
    expect(testCanUseTool('Bash', { command: 'export -p' }).allowed).toBe(false);
  });

  it('blocks declare -x', () => {
    expect(testCanUseTool('Bash', { command: 'declare -x' }).allowed).toBe(false);
  });

  it('blocks /proc/self/environ', () => {
    expect(testCanUseTool('Bash', { command: 'cat /proc/self/environ' }).allowed).toBe(false);
  });

  it('blocks /proc/1/environ', () => {
    expect(testCanUseTool('Bash', { command: 'cat /proc/1/environ' }).allowed).toBe(false);
  });

  it('blocks /proc/42/environ', () => {
    expect(testCanUseTool('Bash', { command: 'strings /proc/42/environ' }).allowed).toBe(false);
  });

  it('blocks os.environ (Python)', () => {
    expect(testCanUseTool('Bash', { command: 'python3 -c "import os; print(os.environ)"' }).allowed).toBe(false);
  });

  it('blocks process.env (Node)', () => {
    expect(testCanUseTool('Bash', { command: 'node -e "console.log(process.env)"' }).allowed).toBe(false);
  });

  it('allows env FOO=bar cmd (legitimate use)', () => {
    expect(testCanUseTool('Bash', { command: 'env FOO=bar node app.js' }).allowed).toBe(true);
  });

  it('allows normal commands', () => {
    expect(testCanUseTool('Bash', { command: 'ls -la' }).allowed).toBe(true);
    expect(testCanUseTool('Bash', { command: 'git status' }).allowed).toBe(true);
    expect(testCanUseTool('Bash', { command: 'npm install' }).allowed).toBe(true);
  });
  ...
  it('blocks $SECRET_VAR from config', () => {
    ...
    const rules = buildContainerSecurityRules(policy);
    expect(testCanUseTool('Bash', { command: 'echo $MY_SECRET' }, false, rules).allowed).toBe(false);
    expect(testCanUseTool('Bash', { command: 'echo ${MY_SECRET}' }, false, rules).allowed).toBe(false);
  });
  ...
  it('blocks http:// URLs', () => {
    expect(testCanUseTool('WebFetch', { url: 'http://example.com' }).allowed).toBe(false);
  });

  it('blocks localhost', () => {
    expect(testCanUseTool('WebFetch', { url: 'https://localhost:3000' }).allowed).toBe(false);
  });

  it('blocks 127.0.0.1', () => {
    expect(testCanUseTool('WebFetch', { url: 'https://127.0.0.1/api' }).allowed).toBe(false);
  });

  it('blocks 10.x private', () => {
    expect(testCanUseTool('WebFetch', { url: 'https://10.0.0.1/admin' }).allowed).toBe(false);
  });

  it('blocks 192.168.x private', () => {
    expect(testCanUseTool('WebFetch', { url: 'https://192.168.1.1/' }).allowed).toBe(false);
  });

  it('blocks [::1] IPv6 loopback', () => {
    expect(testCanUseTool('WebFetch', { url: 'https://[::1]:8080/' }).allowed).toBe(false);
  });

  it('blocks host.docker.internal', () => {
    expect(testCanUseTool('WebFetch', { url: 'https://host.docker.internal:3000' }).allowed).toBe(false);
  });

  it('allows normal HTTPS URLs', () => {
    expect(testCanUseTool('WebFetch', { url: 'https://example.com' }).allowed).toBe(true);
    expect(testCanUseTool('WebFetch', { url: 'https://api.github.com/repos' }).allowed).toBe(true);
  });

  // --- Write/Edit ---

  it('blocks /skills/ path', () => {
    expect(testCanUseTool('Write', { file_path: '/workspace/group/.claude/skills/evil' }).allowed).toBe(false);
  });

  it('blocks CLAUDE.md', () => {
    expect(testCanUseTool('Write', { file_path: '/workspace/group/CLAUDE.md' }).allowed).toBe(false);
  });

  it('blocks settings.json', () => {
    expect(testCanUseTool('Edit', { file_path: '/home/node/.claude/settings.json' }).allowed).toBe(false);
  });

  it('blocks SOUL.md', () => {
    expect(testCanUseTool('Write', { file_path: '/workspace/group/SOUL.md' }).allowed).toBe(false);
  });

  it('allows normal workspace paths', () => {
    expect(testCanUseTool('Write', { file_path: '/workspace/group/notes.md' }).allowed).toBe(true);
  });
  ...
  it('blocks trust-required paths for untrusted', () => {
    ...
    const rules = buildContainerSecurityRules(policy);
    expect(testCanUseTool('Write', { file_path: '/workspace/group/data/file.json' }, false, rules).allowed).toBe(false);
  });
  ...
  it('allows trust-required paths for trusted', () => {
    ...
    policy.write.trust_required_paths = ['/data/'];
    const rules = buildContainerSecurityRules(policy);
    expect(testCanUseTool('Write', { file_path: '/workspace/group/data/file.json' }, true, rules).allowed).toBe(true);
  });
  ...
  it('blocks untrusted tools for untrusted sender', () => {
    ...
    policy.tools.blocked_untrusted = ['mcp__memory__write'];
    const rules = buildContainerSecurityRules(policy);
    expect(testCanUseTool('mcp__memory__write', {}, false, rules).allowed).toBe(false);
  });
  ...
  it('allows untrusted tools for trusted sender', () => {
    ...
    policy.tools.blocked_untrusted = ['mcp__memory__write'];
    const rules = buildContainerSecurityRules(policy);
    expect(testCanUseTool('mcp__memory__write', {}, true, rules).allowed).toBe(true);
  });
});
